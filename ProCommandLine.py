...
def displayUsedLicenses(CON, mainMenuCalled=True):   
    ...
    print()    
    print("1-spatial, 2-3d, 3-network, 4-geostat, 5-datareviewer, 6-workflow")
    print()
    ...
    userSelect = input("Select user [#] or [enter] to return:  ")
...
def inviteUserUI(CON):
    ...
    if cUser == userCheck:
        ...
        while True:
            YesNo = input("  >Requested username not available. Use suggested name: {0}? [y|n|q]  ".format(userCheck))
            if YesNo.lower() == "y":
                cUser = userCheck
    ...
    cEmail = input("Email:  ")     
    cRole = input("Role[{0}]:  ".format("account_user"))     
    ...
    if len(invitedUser) > 0:
        while True:
            assign = input("Do you want to assign them a license now? [y|n]  ")
...
def changeLicense(CON, user=None, entitlements=None):
    ...
    if user is None:
        user = input("User name: [<username> | l(ist users)]  ")
        if user == 'l':
...
def deleteUserUI(CON, user=None):
    ...
    while proceed not in ['Y', 'N']:
        proceed = input("Proceed with deleting << {0} >> [Y|N]  ".format(user))  #NOT casting to lower, must use UPPERCASE letter
...
def searchEUsersUI(CON):    
    ...
    userSearch = input("Enter user name to search: ")
    ...
    if "users" not in userList:
        ...
        userID = input("Select user by ID # to add to Portal:  ")
...
def listEUsersByGroupUI(CON):    
    ...
    group = input("Enter group name to search: ")
    if group == '': return 0
    ...
    if "users" in userList:
        ...
        while True:
            showUsers = input("Found {0} users, do you want to list them [y|n]  ".format(len(userList['users'])))
        ...
        while True:
            addToPortal = input("Do you want to add these users to Portal [y|n]  ")
...
def AddEUserToPortal(CON):
    ...
    userPayload = {'users' : [{'fullname': cName,
                              ...
                              'description': cDesc}]}
    while True:
        cUser = input("   Create user? (this may take a moment) [y|n]  ") 
...
def createCSVUI():
    ...
    location = input("Enter the full path to create the CSV file  ") 
...
if __name__ == "__main__": 
    ...
    username = input("User name: ") or hardUser
